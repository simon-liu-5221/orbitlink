...
- [x] 使用者回饋與評分 — PR #16（PR-12），`feedback` 表累積歷史（非覆蓋），前端彈窗掛在 AppShell
- [x] 管理端：使用者清單、搜尋、停權、查看回饋 — PR #17（AD-01/AD-02），role 欄位 + `require_admin` 沿用既有 JWT auth；AD-03（編輯網站內容）延後，等 GU-02 公開首頁真的存在再做

**驗收**：匯出的 PDF 在另一台機器打開排版正常。**M5 完成**（PR-11 匯出、PR-12 回饋、AD-01/AD-02 管理端）；AD-03 明確延後，見上。
...
- [x] 同一專案多次分析的比較視圖 — PR #18（AN-06 phase 1），`GET /projects/{id}/analyses`
- [x] 情緒、參與度、社群數量的時間走勢 — 同上，Recharts 折線圖 + 純函式（`historyData.ts`）
- [x] 簡單外插預測：只在有 ≥ 5 次歷史分析時啟用，用留出集報告 MAE，並在 UI 明確標示這是外插而非模型預測 — AN-06 phase 2，`GET /projects/{id}/analyses/forecast`，`app/analysis/forecast.py`（線性迴歸 + LOOCV MAE），每個指標各自判斷是否有足夠歷史
- [x] 資料不足時顯示「需要更多歷史分析」而不是編造數字 — `hasEnoughHistory`（< 2 筆不畫趨勢圖）與每指標的「還需要 N 次分析」訊息（< 5 筆可用點不顯示預測）

**驗收**：資料不足時不出現假資料。有 MAE 數字。**M6 完成**（AN-06 phase 1 + phase 2）。
...
- [ ] 情緒模型人工標註驗證，完成 `docs/algorithm-validation.md` §3 — 決定縮小到 150-200 則分層抽樣（不是原訂 500 則，在文件裡誠實記錄樣本數與原因，見下方說明）；工具已完成（`backend/scripts/eval_sentiment/`：抓真實留言 → 分層抽樣 → CLI 標註 → 對詞典替身/真實 XLM-RoBERTa/VADER 三個系統跑 F1），實際標註需要真人判斷，等真的跑過真實頻道、標註完成才能填數字
- [ ] `docs/case-study.md`：挑一個真實頻道做完整分析，寫出你發現了什麼——待決定頻道
- [ ] README 補上截圖與 demo GIF
- [ ]（決定延後，你自己找時間做）錄一段 3 分鐘的 demo 影片——我可以先準備逐字稿/大綱
- [ ]（決定跳過）部署 demo 帳號＋種子資料到正式環境——本機截圖/demo 已足夠展示，且直接寫資料到正式 Neon/Render 的邊際效益低於風險
- [ ]（決定延後，你自己找時間寫）技術部落格——主題已選定：Louvain 多解析度最佳化怎麼做

**驗收**：把 URL 丟給一個不認識這專案的人，他五分鐘內看得懂這是什麼、有什麼發現。

> 情緒驗證決定三個系統都測、對照著寫：目前預設部署的是輕量詞典替身（decision B1），不是 ADR-0004 真正決定的 XLM-RoBERTa（後者要裝 `transformers`+`torch` 才會啟用）。只測其中一個沒辦法誠實回答「使用者實際體驗到的模型有多準」跟「我們決定要用的模型有多準」這兩個不同問題，所以兩個都測、外加 VADER baseline，三個一起放進報告對照。
>
> 過程中發現目前最新的 `transformers`（5.x）沒辦法載入這個 XLM-RoBERTa checkpoint 的 tokenizer（`tiktoken` 轉換路徑的 bug），改用 `transformers==4.46.3` 才正常——已經更新 `app/services/sentiment_transformers.py` 的安裝說明，避免下一個照原本指令做的人卡在一樣的地方。
