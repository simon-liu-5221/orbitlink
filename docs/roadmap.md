...
- [ ] 簡單外插預測：只在有 ≥ 5 次歷史分析時啟用，用留出集報告 MAE，並在 UI 明確標示這是外插而非模型預測 — AN-06 phase 2，獨立 PR
- [x] 資料不足時顯示「需要更多歷史分析」而不是編造數字 — `hasEnoughHistory`，< 2 筆分析時不畫圖

**驗收**：資料不足時不出現假資料。有 MAE 數字（phase 2 完成後）。
